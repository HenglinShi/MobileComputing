...
import com.example.homework1.utils.StringToBitMap
import java.io.File
...
class ModifyReminderActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        var reminderId = intent.getSerializableExtra("reminder") as Int
        val db = AppDatabase.getDatabase(applicationContext, getString(R.string.dbFileName))
        reminder = db.reminderDao().getReminder(reminderId)
    ...
    override fun onResume() {
        super.onResume()
        binding.reminderImageModify.setImageBitmap(StringToBitMap(reminder.image_uri))
        binding.txtTaskDesc.setText(reminder.message)
        binding.txtCreatedDate.setText(reminder.creation_time)
        binding.txtDate.setText(reminder.reminder_time)
        binding.txtLocationX.setText(reminder.location_x.toString())
